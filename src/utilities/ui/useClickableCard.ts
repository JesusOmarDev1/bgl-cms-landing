...
/**
 * Hook to make a card clickable by detecting clicks and navigating to the link
 * @param external - Whether the link is external
 * @param newTab - Whether to open in a new tab
 * @param scroll - Whether to scroll to top on navigation
 * @returns Card and link refs
 */
        ...
        if (link.current?.href && difference <= 250) {
          if (!hasActiveParent.current && pressedButton.current === 0 && !e.ctrlKey) {
            if (external) {
              ...
              router.push(link.current.href as any, { scroll })
            }
          }
        }
      }
